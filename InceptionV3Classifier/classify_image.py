...
class NodeLookup(object):
    ...
    def __init__(self,
        ...
        if not label_lookup_path:
            label_lookup_path = os.path.join(pathlib.Path(os.path.abspath(
                __file__)).parent, 'imagenet_2012_challenge_label_map_proto.pbtxt')
        if not uid_lookup_path:
            uid_lookup_path = os.path.join(pathlib.Path(os.path.abspath(
                __file__)).parent, 'imagenet_synset_to_human_label_map.txt')
...
def create_graph():
    ...
    model_file = os.path.join(pathlib.Path(os.path.abspath(
        __file__)).parent, "classify_image_graph_def.pb")
...
def run_inference_on_image(image_url):
    ...
    req_img = urllib.request.Request(
        image_url, headers={'User-Agent': user_agent_header})
